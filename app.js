...
// 同月重复行动的收益/风险修正。只在 applyAction 执行期间偏离 1，事件与剧情调用 gainStat 时不受影响。
...
const ACTIONS=[
  {id:"train_box",style:"box",phases:["academy","firstteam","overseas","campus","pro"],icon:"◎",name:"抢点终结",desc:"一筐球摆在禁区弧顶，射门、补射、逆足，再把定位球放在同一处反复打。",effects:["射术↑↑","定位球↑","意志↑","体能↓"],max:2,run:s=>{gainSkill(s,"finishing",.75,"finish");gainSkill(s,"setPiece",.5,"setpiece");gainStat(s,"will",.22,"will");change(s,"fitness",-10)}},
  {id:"train_burst",style:"burst",phases:["academy","firstteam","overseas","campus","pro"],icon:"»",name:"速度突破",desc:"折返冲刺、启动步、连续变向——用乳酸换那更快的第一步。",effects:["速度↑↑","爆发↑↑","盘带↑","体能↓"],max:2,run:s=>{gainStat(s,"speed",.7,"speed");gainStat(s,"burst",.62,"burst");gainSkill(s,"dribble",.4,"dribble");change(s,"fitness",-13);fatigueInjuryCheck(s,.022)}},
  {id:"train_target",style:"target",phases:["academy","firstteam","overseas","campus","pro"],icon:"▲",name:"对抗支点",desc:"负重、卡位、争顶，跟队里最壮的中卫一次次撞在一起。",effects:["对抗↑↑","耐力↑↑","意志↑","体能↓"],max:2,run:s=>{gainStat(s,"height",.55,"height");gainStat(s,"stamina",.6,"stamina");gainStat(s,"will",.3,"will");change(s,"fitness",-13);fatigueInjuryCheck(s,.022)}},
  {id:"train_play",style:"play",phases:["academy","firstteam","overseas","campus","pro"],icon:"▣",name:"回撤组织",desc:"跟着全队跑位置串联，赛后再把录像一帧帧倒回去看自己该往哪儿撤。",effects:["视野↑↑","盘带↑","教练信任↑","状态↑"],max:2,run:s=>{gainSkill(s,"vision",.62,"vision");gainSkill(s,"dribble",.35,"tactics");gainSkill(s,"setPiece",.3,"setpiece");gainSkill(s,"finishing",.15,"training");change(s,"coachFavor",hasTalent(s,"training_rat")?5:3);change(s,"form",3);change(s,"fitness",-6);if(hasTalent(s,"training_rat")&&chance(.18)){gainSkill(s,"vision",.5);log(s,"good","训练模范发动：你留下加练的细节被教练看见。")}}},
  {id:"love_time",phases:["academy","firstteam","overseas","campus","pro"],icon:"♥",name:"陪小满",desc:"同城就绕远路陪她走一段，异地就隔着屏幕把今天讲给对方听。",effects:["感情↑","心情↑","意志↑"],max:1,show:s=>["恋人","暧昧","异地"].includes(s.relationship.status),run:s=>{const away=s.relationship.status==="异地";changeLove(s,hasTalent(s,"childhood_bond")?10:7);change(s,"morale",7);gainStat(s,"will",.3,"love");if(!away&&chance(.25)){change(s,"coachFavor",-3);log(s,"warn","你迟到了七分钟。周骁没有骂你，只在名单上画了一个圈。")}else if(away)change(s,"coachFavor",-1)}},
  ...
  {id:"home",phases:["academy","firstteam","overseas","campus","pro"],icon:"⌂",name:"顾家",desc:"陪父母吃顿饭；手头宽裕就打点钱回去，让父亲少上几天夜班。",effects:["家庭↑","意志↑","心情↑"],max:1,run:s=>{change(s,"family",9);gainStat(s,"will",.28,"will");change(s,"morale",6);if(s.debt)s.debt=Math.max(0,s.debt-2)}},
  ...
  {id:"street",phases:["academy","campus"],icon:"✦",name:"去踢野球",desc:"没有战术板，只有狭小场地和不服输的对手。",effects:["盘带↑↑","创造力↑","受伤/纪律风险"],max:1,run:s=>{gainSkill(s,"dribble",1,"dribble");gainSkill(s,"vision",.35,"vision");change(s,"coachFavor",-3);change(s,"fitness",-11);fatigueInjuryCheck(s,.032)}},  {id:"campus_match",phases:["campus"],icon:"旗",name:"校队强化赛",desc:"职业通道变窄了，但球场没有消失。",effects:["技术↑","球探关注↑","体能↓"],max:2,run:s=>{gainSkill(s,"finishing",.55,"finish");gainSkill(s,"dribble",.45,"dribble");change(s,"scout",5);change(s,"fitness",-12)}},
...
const EVENTS=[
  ...
  {id:"teammate_blame",phase:["academy","firstteam","overseas"],title:"队友把丢球算在你头上",body:"<p>训练赛最后三十秒，你在前场被断，对方打了一个反击，1比0。</p><p>更衣室里没人说话。水声，鞋钉敲地板的声音。直到有人——不是最大声的那个，但也不是最小声的——从柜子那头丢过来一句：<span class='dialogue'>“有些人，集锦够用了，比赛够呛。”</span></p><p>所有人都听见了。有人低头看手机，有人假装在解鞋带。你站在柜子前面，背对着所有人，能感觉到背后的视线。你在等他再说一句。</p><p>他没有说。他等你说。</p>",portrait:"assets/coach-zhou.webp",options:s=>[
    option("当场顶回去","意志+1，队内地位可能上升；教练信任波动",()=>{gainStat(s,"will",1,"pressure");change(s,"coachFavor",chance(.5)?5:-5);log(s,"story","你转过身。没等你开口，队长先站起来挡在你面前：“丢球是全队的事，有话说清楚。”更衣室安静了三秒，有人把柜门关上了。")}),
    option("承认丢球，要求一起复盘","视野+1，教练信任+5；状态-3",()=>{gainSkill(s,"vision",1,"vision");change(s,"coachFavor",5);change(s,"form",-3);log(s,"story","你说：“那个球是我的。但我想全队一起看录像，那个反击是怎么打穿中场的。”靠窗的队友把手机放下了。")})]},
  ...
  {id:"red_card_choice",phase:["firstteam","overseas","pro"],title:"队友被恶意铲倒，全队都在看你",body:"<p>对方中卫那一脚踩在你队友脚踝上，他没出声，但在地上翻了一圈才站起来。裁判只给了黄牌。队长经过你身边时丢了一句：<span class='dialogue'>“别冲动。”</span></p><p>下一个回合，球到了你脚下。你背身拿球，那个中卫贴上来，胸口顶住你的后腰，下巴搁在你肩膀上，用只有你们两个人听得见的音量说：<span class='dialogue'>“你想怎么样？”</span></p><p>看台上有人在吹口哨，有人在喊。你的手已经攥起来了。球在你脚下，他在你身后，裁判在十米外。</p><p>所有人在等你选。</p>",options:s=>[
    option("用下一次进攻回应","意志+1，状态+4；需要压住情绪",()=>{gainStat(s,"will",1,"clutch");change(s,"form",4);log(s,"story","你把球做出去，前插，接球，不跟他纠缠。两分钟后你用一个变向把他晃倒在地，然后把球传了出去。你没看他，但你知道他在看你。")}),
...
const STORY_BEATS={
  ...
  42:s=>({title:"父亲病床边的终场哨",portrait:"assets/father.webp",body:`<p>你赶到病房的时候，你爸刚做完一轮透析。他闭着眼，脸上只剩一层皮贴着骨头。你坐在床边，不知道该说什么。</p><p>沉默了很久，他突然开口，声音哑得几乎听不见：<span class="dialogue">“今天……有比赛？”</span>你说推迟了。他摇了摇头：<span class="dialogue">“别推。我这一辈子，就是推得太多了。”</span></p><p>他转过头看着你，目光浑浊，但焦距是准的：<span class="dialogue">“上场去。我等你回来再说。”</span></p><p>你没有走，一直坐到护士来换药。你走到门口时他好像睡着了，你听见他在背后说了一句很轻的话——<span class="dialogue">“踢给爸看。”</span></p>`,options:[option("承担治疗，拒绝灰色资金","现金-18万，家庭+15；意志+2",()=>{addMoney(s,-18);change(s,"family",15);gainStat(s,"will",2,"will")}),option("让父母接受保险与社会援助","家庭+8；隐私/媒体风险+6",()=>{change(s,"family",8);change(s.risks,"media",6)})]}),
...
const MOMENTS=[
  {id:"counter_break",title:"反越位单刀",min:55,max:75,
   ...
   options:[
     {text:"冷静推射",tip:"成功率稳定，依赖射术",stat:"finishing",risk:"safe",style:"box",goal:true,up:.35,down:-.2,
      win:"你把重心压住，脚弓一推，皮球贴着草皮钻进远角。",fail:"你选了最稳的一脚，可惜球擦着立柱滑出底线。"},
     {text:"过掉门将",tip:"成功率低，成功后评分大涨，依赖盘带",stat:"dribble",risk:"bold",style:"burst",goal:true,up:.8,down:-.5,
      win:"你一个扣球晃倒门将，把球稳稳推进空门。看台先是安静了半秒，然后炸了。",fail:"你想多带一步，门将扑在你脚下把球没收。机会没了。"},
     {text:"横传队友",tip:"容易成功，可能获得助攻与教练信任",stat:"vision",risk:"safe",style:"play",assist:true,up:.3,down:-.2,favor:2,
      win:"你没有贪功，一脚横敲，队友面对空门推空门。",fail:"传球慢了半拍，回追的后卫把线路封死。"},
     {text:"强行突破",tip:"爆点前锋Ⅱ · 硬吃门将，成功即进球",stat:"burst",risk:"bold",style:"burst",need:"burst",goal:true,up:.9,down:-.45,
  ...
  {id:"box_scramble",title:"禁区混战",min:20,max:85,cond:s=>s.matchPlan==="box"||styleOf(s,"box")>=1,
   ...
   options:[
     {text:"稳住等落点",tip:"依赖意志，稳但收益小",stat:"will",risk:"safe",style:"box",goal:true,up:.25,down:-.15,
      win:"你没有急着捅，等球落下来才推射，皮球从两条腿的缝里钻进球门。",fail:"你等得太久，后卫先一步把球捅出禁区。"},
     {text:"贴身垫射",tip:"依赖射术",stat:"finishing",risk:"none",style:"box",goal:true,up:.6,down:-.3,
      win:"你伸脚就捅，球从门将腋下滚进去。不好看，但进了。",fail:"你伸脚够到了球，方向却完全不对。"},
     {text:"做球给后插上",tip:"依赖视野",stat:"vision",risk:"none",style:"play",assist:true,up:.3,down:-.2,
      win:"你用脚后跟一磕，后插上的队友迎球爆射。",fail:"你想做球，队友却没跟上你的想法。"},
     {text:"抢前点",tip:"禁区杀手Ⅱ · 抢在所有人之前",stat:"finishing",risk:"bold",style:"box",need:"box",goal:true,up:.85,down:-.35,
  ...
  {id:"wing_duel",title:"边路一对一",min:15,max:80,
   ...
   options:[
     {text:"底线传中",tip:"依赖视野",stat:"vision",risk:"safe",style:"play",assist:true,up:.3,down:-.2,
      win:"你把球送到底线，回敲的弧线正好绕过第一落点。",fail:"传中被前点的中卫解围。"},
     {text:"强行内切",tip:"依赖盘带，风险高",stat:"dribble",risk:"bold",style:"burst",goal:true,up:.7,down:-.4,
      win:"你连续两次变向切进肋部，起脚打远角。",fail:"第一脚触球稍大，边后卫提前卡住了线路。"},
     {text:"回做保球权",tip:"依赖意志，稳妥",stat:"will",risk:"safe",up:.15,down:-.1,
      win:"你把球稳稳回做，全队重新组织。教练在场边点了下头。",fail:"回做的球力量不够，被对手抢断。"}]},

  {id:"aerial_duel",title:"角球争顶",min:25,max:85,cond:s=>s.stats.height>=60||styleOf(s,"target")>=1,
   body:"第{minute}分钟，角球旗附近，队友举手示意。你在禁区里找位置，身边的中卫比你高半个头。",
   options:[
     {text:"后点包抄",tip:"依赖对抗",stat:"height",risk:"none",style:"target",goal:true,up:.55,down:-.2,
      win:"你绕到后点，几乎无人盯防，头球轻轻一蹭改变了方向。",fail:"你抢到了落点，头球却高出横梁。"},
     {text:"抢前点冲顶",tip:"依赖对抗，风险高",stat:"height",risk:"bold",style:"target",goal:true,up:.8,down:-.25,
      win:"你从中卫身前挤了出去，高高跃起把球砸进网窝。",fail:"被中卫贴住身体，你没能顶上力量。"},
     {text:"做墙掩护",tip:"为队友创造空间",stat:"will",risk:"safe",style:"target",assist:true,up:.25,down:-.1,favor:2,
      win:"你把中卫牢牢挡在身后，队友从你身边跑出来完成攻门。",fail:"你的掩护被裁判判成推人。"}]},

  {id:"free_kick",title:"禁区前沿任意球",min:20,max:88,cond:s=>s.skills.setPiece>=55||hasTalent(s,"free_kick"),
   body:"第{minute}分钟，你在禁区前沿被放倒。人墙正在排，队友把球摆好，抬头看你。",
   options:[
     {text:"直接射门",tip:"依赖定位球，风险高",stat:"setPiece",risk:"bold",style:"box",goal:true,up:.9,down:-.3,
      win:"皮球越过人墙急速下坠，直挂死角。门将连动都没动。",fail:"球越过了人墙，却被门将侧扑封出。"},
     {text:"传中找中锋",tip:"依赖视野",stat:"vision",risk:"none",style:"play",assist:true,up:.35,down:-.2,
      win:"你把弧线吊到后点，中锋高高跃起。",fail:"传球被人墙里冲出来的后卫解围。"},
     {text:"短传配合",tip:"依赖视野，稳妥",stat:"vision",risk:"safe",style:"play",up:.15,down:-.1,
  ...
  {id:"hold_up",title:"背身拿球",min:10,max:80,
   ...
   options:[
     {text:"回敲组织",tip:"依赖视野",stat:"vision",risk:"safe",style:"play",assist:true,up:.25,down:-.15,
      win:"你把球回敲给插上的中场，他一脚直塞撕开了防线。",fail:"回敲的力量不对，球被断在中场。"},
     {text:"扛住等支援",tip:"依赖耐力，稳妥",stat:"stamina",risk:"safe",style:"target",up:.15,down:-.1,
      win:"你用身体把球护了整整六秒，直到两名队友跑到位。",fail:"你被顶得失去重心，球滚出边线。"},
     {text:"转身强突",tip:"依赖爆发，风险高",stat:"burst",risk:"bold",style:"burst",goal:true,up:.7,down:-.4,
      win:"你一个转身直接把中卫甩在原地，杀进禁区完成攻门。",fail:"你想转身，中卫的脚已经先一步伸到了球上。"},
     {text:"背身护球",tip:"支点中锋Ⅱ · 用身体做支点",stat:"stamina",risk:"safe",style:"target",need:"target",teamGoal:true,up:.3,down:-.1,favor:3,
  ...
  {id:"late_chase",title:"落后追分",min:78,max:88,forced:true,
   ...
   options:[
     {text:"组织最后一攻",tip:"依赖视野",stat:"vision",risk:"none",style:"play",assist:true,up:.4,down:-.25,
      win:"你没有自己来，一脚过顶球找到了后点完全无人盯防的队友。",fail:"你想找人，但所有人都被盯死了。"},
     {text:"单点强攻",tip:"依赖射术，风险高",stat:"finishing",risk:"bold",style:"box",goal:true,up:1.2,down:-.4,
      win:"你在人堆里硬生生找到一个射门角度，皮球擦着门柱内侧进了。",fail:"你的射门被封堵，反弹出禁区。"},
     {text:"后插上远射",tip:"依赖定位球，成功即经典",stat:"setPiece",risk:"bold",style:"box",goal:true,classic:true,up:1.2,down:-.35,
  ...
  {id:"defend_lead",title:"领先保胜果",min:78,max:88,forced:true,
   ...
   options:[
     {text:"回撤参与防守",tip:"依赖耐力，教练最想看到的",stat:"stamina",risk:"safe",style:"target",up:.3,down:-.1,favor:4,fitCost:5,
      win:"你退到本方禁区前沿参与卡位，最后十分钟跑了整整两公里。教练在场边看着你。",fail:"你回撤了，但跑动已经跟不上节奏。"},
     {text:"打反击扩大比分",tip:"依赖速度",stat:"speed",risk:"none",style:"burst",goal:true,up:.8,down:-.25,
      win:"抢断之后你一个人带球奔袭四十米，从容推射空门。",fail:"反击的最后一传被回追的后卫破坏。"},
     {text:"控球拖时间",tip:"依赖盘带",stat:"dribble",risk:"safe",style:"burst",up:.2,down:-.2,favor:2,
  ...
  {id:"press_trigger",title:"高位逼抢",min:10,max:70,cond:s=>s.matchPlan==="press",
   ...
   options:[
     {text:"封堵传球线路",tip:"依赖视野",stat:"vision",risk:"none",style:"play",assist:true,up:.3,down:-.15,favor:3,fitCost:5,
      win:"你切断了门将和后卫之间那条线，逼得他大脚开出，队友轻松拿到二点球。",fail:"你封错了方向，对手从你身后把球做了出去。"},
     {text:"上抢门将出球",tip:"依赖爆发，风险高",stat:"burst",risk:"bold",style:"burst",goal:true,up:1,down:-.3,fitCost:8,
      win:"你在门将出脚的瞬间把球封了下来，皮球弹进空门。全场哗然。",fail:"你扑了个空，对手就地打起反击。"},
     {text:"保留体力回位",tip:"依赖耐力，稳妥",stat:"stamina",risk:"safe",up:.1,down:-.1,fitGain:4,
  ...
  {id:"through_ball",title:"面向球门接球",min:15,max:80,cond:s=>s.matchPlan==="deep"||styleOf(s,"play")>=1,
   ...
   options:[
     {text:"稳妥横传",tip:"依赖视野，不冒险",stat:"vision",risk:"safe",style:"play",up:.15,down:-.1,
      win:"你把球转移到另一侧，防线被扯动了一下。",fail:"横传力量不够，被中场拦下。"},
     {text:"自己带球推进",tip:"依赖盘带，风险高",stat:"dribble",risk:"bold",style:"burst",goal:true,up:.5,down:-.3,
      win:"你连过两人杀到禁区弧顶，起脚攻门。",fail:"你带球过半场，第三个人把你断了。"},
     {text:"直接远射",tip:"依赖射术，风险高",stat:"finishing",risk:"bold",style:"box",goal:true,up:.75,down:-.3,
      win:"你根本没往前带，抬脚就是一记三十米重炮，直挂上角。",fail:"你抡了一脚，球高出横梁很多。"},
     {text:"回撤直塞",tip:"组织型Ⅱ · 一脚打穿防线",stat:"vision",risk:"none",style:"play",need:"play",assist:true,teamGoal:true,up:.6,down:-.2,
...
function momentSuccessRate(s,m,o,opp,behind){
  const statVal=o.stat in s.skills?s.skills[o.stat]:s.stats[o.stat]||50;
  let p=.35+(statVal-opp)*.008+s.form/1000+s.fitness/2000+MOMENT_RISK[o.risk];
  if(hasTalent(s,"box_instinct")&&o.stat==="finishing")p+=.06;
  if(hasTalent(s,"explosive_start")&&(o.stat==="dribble"||o.stat==="burst"))p+=.06;
  if(hasTalent(s,"aerial_king")&&o.stat==="height")p+=.08;
  if(hasTalent(s,"free_kick")&&o.stat==="setPiece")p+=.1;
  if(hasTalent(s,"football_iq")&&o.stat==="vision")p+=.05;
  if(hasTalent(s,"big_heart")&&behind)p+=.05;
  if(styleOf(s,"box")>=1&&o.stat==="finishing")p+=.05;
  if(styleOf(s,"burst")>=1&&(m.id==="counter_break"||m.id==="press_trigger"))p+=.06;
  if(styleOf(s,"box")>=3&&m.id==="late_chase"&&o.stat==="finishing")p+=.08;
...
function prepareMatch(s,rng=Math.random,opts={}){
  ...
  if(plays){
    ...
    for(let i=0;i<attempts;i++){
      ...
      const isAssist=success&&["pass","dribble"].includes(type)&&assists+goals<gf&&rng()<clamp(.26+(s.skills.vision-45)/150+assistBonus,.18,.63);
...
function finishMatch(s,pending,rng=Math.random){
  ...
  p.moments.forEach((slot,i)=>{
    ...
    if(ok){
      ...
      if(o.goal){
        if(claimGoal(bold)){
          if(styleOf(s,"box")>=3&&m.id==="late_chase"&&o.stat==="finishing")up+=.3;
          if(styleOf(s,"burst")>=3&&o.stat==="dribble"&&rng()<.12&&claimGoal(bold))
...
function applyAction(id){if(!S||S.actionPoints<=0)return;const a=ACTIONS.find(x=>x.id===id);if(!a||!a.phases.includes(phaseOf(S))||(a.show&&!a.show(S)))return;const used=S.actionUsage[id]||0;if(used>=(a.max||1))return;if(a.cost&&(S.money||0)<a.cost){toast("资金不足，先靠比赛或媒体活动赚钱");return}if(S.injury.months>0&&!['recover','home','english','love_time','gift'].includes(id)){toast("伤停期不能完成高强度行动");return}
  ...
  unlock("first_action");if(S.flags.weeklyPromise&&id==="train_play"&&used===0){gainSkill(S,"finishing",-.08,"finish")};const fb=actionFeedback(a);S.lastActionFeedback={name:a.name,text:fb,effects:a.effects.join("、")};log(S,"action",fb);checkCombos(S);checkAchievements(S);saveGame();renderAll()}

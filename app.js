...
const ACTIONS=[
  {id:"train_body",phases:["academy","firstteam","overseas","campus","pro"],icon:"»",name:"身体训练",desc:"冲刺、折返、力量与核心——用乳酸换更快的第一步和更稳的对抗。",effects:["速度↑","爆发↑","耐力/对抗↑","体能↓"],max:2,run:s=>{gainStat(s,"speed",.55,"speed");gainStat(s,"burst",.5,"burst");gainStat(s,"stamina",.35,"stamina");gainStat(s,"height",.3,"height");change(s,"fitness",-13);fatigueInjuryCheck(s,.024)}},
  {id:"train_tech",phases:["academy","firstteam","overseas","campus","pro"],icon:"◎",name:"技术训练",desc:"射门、抢点、逆足，再把定位球摆在同一处反复打。",effects:["射术↑","定位球↑","意志↑","体能↓"],max:2,run:s=>{gainSkill(s,"finishing",.7,"finish");gainSkill(s,"setPiece",.55,"setpiece");gainStat(s,"will",.24,"will");change(s,"fitness",-10)}},
  {id:"train_team",phases:["academy","firstteam","overseas","campus","pro"],icon:"▣",name:"合练与复盘",desc:"跟着全队把基本功做标准，赛后再把录像一帧帧倒回去看。",effects:["视野↑","盘带↑","教练信任↑","状态↑"],max:2,run:s=>{gainSkill(s,"vision",.6,"vision");gainSkill(s,"dribble",.35,"tactics");gainSkill(s,"finishing",.2,"training");gainStat(s,"stamina",.2,"stamina");change(s,"coachFavor",hasTalent(s,"training_rat")?5:3);change(s,"form",3);change(s,"fitness",-6);if(hasTalent(s,"training_rat")&&chance(.18)){gainSkill(s,"vision",.5);log(s,"good","训练模范发动：你留下加练的细节被教练看见。")}}},
...
function simulateMatchCore(s,rng=Math.random,opts={}){
  ...
  const injuryChance=plays?Math.max(.005,(.016+(45-s.fitness)/500+(s.injury.risk||0)/900-(hasTalent(s,"iron_man")?.015:0))*diffOf(s).injury*assetInjuryFactor(s)):0;
...
const ACTION_FEEDBACK={
  train_body:["最后一组冲刺你几乎要吐，但落地那一下，第一步明显更快了。","杠铃一次次压下来，你能感觉到，下次中卫再靠上来，你站得住了。"],
  train_tech:["近角、远角、抢点、逆足……你一颗颗把球送进网窝，直到手感发烫。","同一个落点摆了上百次，弧线越来越听你的话。"],
  train_team:["合练结束别人都走了，你又多跑了两趟折返，训练服湿得能拧出水。","录像一帧帧倒回去看，你终于弄明白那个球为什么没接到。"],
...
function applyAction(id){if(!S||S.actionPoints<=0)return;const a=ACTIONS.find(x=>x.id===id);if(!a||!a.phases.includes(phaseOf(S))||(a.show&&!a.show(S)))return;const used=S.actionUsage[id]||0;if(used>=(a.max||1))return;if(a.cost&&(S.money||0)<a.cost){toast("资金不足，先靠比赛或媒体活动赚钱");return}if(S.injury.months>0&&!['recover','home','english','love_time','gift'].includes(id)){toast("伤停期不能完成高强度行动");return}
  S.actionPoints--;S.actionUsage[id]=used+1;a.run(S);unlock("first_action");if(S.flags.weeklyPromise&&id==="train_team"&&used===0){gainSkill(S,"finishing",-.08,"finish")};const fb=actionFeedback(a);S.lastActionFeedback={name:a.name,text:fb,effects:a.effects.join("、")};log(S,"action",fb);checkAchievements(S);saveGame();renderAll()}

...
const ACTIONS=[
  {id:"train_box",style:"box",phases:["academy","firstteam","overseas","campus","pro"],icon:"◎",name:"抢点终结",desc:"一筐球摆在禁区弧顶，射门、补射、逆足，再把定位球放在同一处反复打。",effects:["射术↑↑","定位球↑","意志↑","体能↓"],max:2,run:s=>{gain(s,"SHO",.75,"finish");gain(s,"PAS",.5,"setpiece");gain(s,"WIL",.22,"will");change(s,"fitness",-10)}},
  {id:"train_burst",style:"burst",phases:["academy","firstteam","overseas","campus","pro"],icon:"»",name:"速度突破",desc:"折返冲刺、启动步、连续变向——用乳酸换那更快的第一步。",effects:["速度↑↑","爆发↑↑","盘带↑","体能↓"],max:2,run:s=>{gain(s,"PAC",.7,"speed");gain(s,"PAC",.62,"burst");gain(s,"DRI",.4,"dribble");change(s,"fitness",-13);fatigueInjuryCheck(s,.022)}},
  {id:"train_target",style:"target",phases:["academy","firstteam","overseas","campus","pro"],icon:"▲",name:"对抗支点",desc:"负重、卡位、争顶，跟队里最壮的中卫一次次撞在一起。",effects:["对抗↑↑","耐力↑↑","意志↑","体能↓"],max:2,run:s=>{gain(s,"PHY",.55,"height");gain(s,"PHY",.6,"stamina");gain(s,"WIL",.3,"will");change(s,"fitness",-13);fatigueInjuryCheck(s,.022)}},
  {id:"train_play",style:"play",phases:["academy","firstteam","overseas","campus","pro"],icon:"▣",name:"回撤组织",desc:"跟着全队跑位置串联，赛后再把录像一帧帧倒回去看自己该往哪儿撤。",effects:["视野↑↑","盘带↑","教练信任↑","状态↑"],max:2,run:s=>{gain(s,"PAS",.62,"vision");gain(s,"DRI",.35,"tactics");gain(s,"PAS",.3,"setpiece");gain(s,"SHO",.15,"training");change(s,"coachFavor",hasTalent(s,"training_rat")?5:3);change(s,"form",3);change(s,"fitness",-6);if(hasTalent(s,"training_rat")&&chance(.18)){gain(s,"PAS",.5);log(s,"good","训练模范发动：你留下加练的细节被教练看见。")}}},
  {id:"love_time",phases:["academy","firstteam","overseas","campus","pro"],icon:"♥",name:"陪小满",desc:"同城就绕远路陪她走一段，异地就隔着屏幕把今天讲给对方听。",effects:["感情↑","心情↑","意志↑"],max:1,show:s=>["恋人","暧昧","异地"].includes(s.relationship.status),run:s=>{const away=s.relationship.status==="异地";changeLove(s,hasTalent(s,"childhood_bond")?10:7);change(s,"morale",7);gain(s,"WIL",.3,"love");if(!away&&chance(.25)){change(s,"coachFavor",-3);log(s,"warn","你迟到了七分钟。周骁没有骂你，只在名单上画了一个圈。")}else if(away)change(s,"coachFavor",-1)}},
  ...
  {id:"home",phases:["academy","firstteam","overseas","campus","pro"],icon:"⌂",name:"顾家",desc:"陪父母吃顿饭；手头宽裕就打点钱回去，让父亲少上几天夜班。",effects:["家庭↑","意志↑","心情↑"],max:1,run:s=>{change(s,"family",9);gain(s,"WIL",.28,"will");change(s,"morale",6);if(s.debt)s.debt=Math.max(0,s.debt-2)}},
  {id:"recover",phases:["academy","firstteam","overseas","campus","pro"],icon:"✚",name:"恢复",desc:"睡到自然醒，加上冰浴、理疗和睡眠监测——不上集锦，却让身体更耐用。",effects:["体能↑↑","伤病恢复","降风险"],max:2,run:s=>{change(s,"fitness",22+(hasTalent(s,"quick_healer")?6:0));change(s,"morale",5);s.injury.risk=Math.max(0,(s.injury.risk||0)-6);if(s.injury.months>0){s.injury.months=Math.max(0,s.injury.months-(hasTalent(s,"quick_healer")?2:1));if(!s.injury.months){s.injury.name="";unlock("injury_return");log(s,"good","康复评估通过，你重新回到完整训练。")}}}},
  ...
  {id:"street",phases:["academy","campus"],icon:"✦",name:"去踢野球",desc:"没有战术板，只有狭小场地和不服输的对手。",effects:["盘带↑↑","创造力↑","受伤/纪律风险"],max:1,run:s=>{gain(s,"DRI",1,"dribble");gain(s,"PAS",.35,"vision");change(s,"coachFavor",-3);change(s,"fitness",-11);fatigueInjuryCheck(s,.032)}},  {id:"campus_match",phases:["campus"],icon:"旗",name:"校队强化赛",desc:"职业通道变窄了，但球场没有消失。",effects:["技术↑","球探关注↑","体能↓"],max:2,run:s=>{gain(s,"SHO",.55,"finish");gain(s,"DRI",.45,"dribble");change(s,"scout",5);change(s,"fitness",-12)}},
  ...
  {id:"national_role",phases:["pro"],icon:"★",name:"适应多位置",desc:"为国家队练习边锋和影锋职责，俱乐部训练会被分走。",effects:["国家队适配↑","视野↑","俱乐部状态↓"],max:1,show:s=>s.national.called,run:s=>{change(s.national,"adapt",9);gain(s,"PAS",.4,"tactics");change(s,"form",-2)}}
];
...
const EVENTS=[
  ...
  {id:"father_boots",once:true,phase:["academy"],title:"父亲买了一双你不敢穿坏的球鞋",body:"<p>鞋盒放在你床上，不是快递包装，是店里的手提袋。你打开，一双顶级款刺客——你上周在店里隔着玻璃看过的那双。你拎起来试了一下，刚好合脚。鞋仓里还垫着旧报纸，你爸的习惯。</p><p>你走出房间，他坐在客厅里修遥控器，头没抬：<span class='dialogue'>“旧鞋我扔了。底都磨穿了，穿着容易受伤。”</span></p><p>你没有拆穿——那双旧鞋你上周自己藏起来了，根本没给他看见过。他一定是翻了你柜子。</p><p>你把新鞋放回鞋盒，又把旧鞋从柜子里翻出来：大底已经开胶，鞋头那块皮磨得发白，是你练颠球磨出来的。你没有穿新鞋，但你把鞋盒放在床头，放了很久。</p>",portrait:"assets/father.webp",options:s=>[
    option("收下新鞋，把旧鞋留作纪念","爆发+1，家庭+5；父亲继续加班",()=>{gain(s,"PAC",1,"burst");change(s,"family",5);s.risks.familyFatigue=(s.risks.familyFatigue||0)+7}),
  ...
  {id:"teammate_blame",phase:["academy","firstteam","overseas"],title:"队友把丢球算在你头上",body:"<p>训练赛最后三十秒，你在前场被断，对方打了一个反击，1比0。</p><p>更衣室里没人说话。水声，鞋钉敲地板的声音。直到有人——不是最大声的那个，但也不是最小声的——从柜子那头丢过来一句：<span class='dialogue'>“有些人，集锦够用了，比赛够呛。”</span></p><p>所有人都听见了。有人低头看手机，有人假装在解鞋带。你站在柜子前面，背对着所有人，能感觉到背后的视线。你在等他再说一句。</p><p>他没有说。他等你说。</p>",portrait:"assets/coach-zhou.webp",options:s=>[
    ...
    option("承认丢球，要求一起复盘","视野+1，教练信任+5；状态-3",()=>{gain(s,"PAS",1,"vision");change(s,"coachFavor",5);change(s,"form",-3);log(s,"story","你说：“那个球是我的。但我想全队一起看录像，那个反击是怎么打穿中场的。”靠窗的队友把手机放下了。")})]},
  {id:"mystery_supplement",phase:["academy","firstteam","overseas","pro"],title:"一瓶“绝对查不出来”的补剂",body:"<p>训练结束后，一个平时不怎么跟你说话的人从包里拿出一瓶东西，放在你凳子上。没有完整中文标签，瓶身是哑光白，上面只有一行英文，没写成分。</p><p>他说：<span class='dialogue'>“体能师配的，恢复快，绝对查不出来。”</span>你问他多少钱。他说不用钱，<span class='dialogue'>“你先试，有用再说。”</span></p><p>他走之后你拿起那瓶东西晃了一下，液体，没有味道。瓶口封膜完好，但封膜下面的压印不太平整。</p><p>队医办公室的门还亮着。走廊里没有人。你把这瓶东西放进了柜子，没有扔，也没有用。</p>",options:s=>[
    ...
    option("只拿去检测，不供出队友","花费4万；意志+1，教练无变化",()=>{addMoney(s,-4);gain(s,"WIL",1,"will");change(s.risks,"health",-4)})]},
  ...
  {id:"growth_pain",once:true,phase:["academy"],title:"一个夏天，你突然长高了六厘米",body:"<p>你撞到了门框。不是开玩笑那种撞，是真的没估好高度，额头磕在上沿，整个人往后倒了一步。</p><p>队医量完身高，在表上改了数字。新的体重跟不上新的身高——你在禁区里接一个半高球，身体已经做出了转身的动作，脚却还在原地。</p><p>教练在笔记上写了两行，训练结束后把你叫到一边：<span class='dialogue'>“两条路。要么去禁区里待着，背身拿球，靠身高吃饭；要么花三个月重新建立协调性，但这段时候你可能连替补席都坐不上。”</span></p><p>你站在门口，出门的时候又低了低头。</p>",portrait:"assets/coach-zhou.webp",options:s=>[
    option("改造成支点中锋","身高/对抗+2，射术+1；速度-1",()=>{s.heightCm=Math.min(200,s.heightCm+6);gain(s,"PHY",2,"height");gain(s,"SHO",1,"finish");s.attrs.PAC=clamp(s.attrs.PAC-1,1,99)}),
    option("花时间重建协调性","盘带+2，爆发+1；未来2个月教练信任-4",()=>{s.heightCm=Math.min(200,s.heightCm+6);gain(s,"DRI",2,"dribble");gain(s,"PAC",1,"burst");change(s,"coachFavor",-4)})]},
  ...
  {id:"firstteam_veteran",once:true,phase:["firstteam","pro"],title:"老队员要你训练后留下来捡球",body:"<p>训练刚结束，一瓶水从长凳那头滚到你脚下。<span class='dialogue'>“新人都这样。球收好，圈收好，垃圾带出去。”</span></p><p>说话的人已经换好拖鞋了。他比你大六岁，一线队出场次数比你多两位数。他不是在跟你商量。</p><p>你蹲下来，把那瓶水捡起来放在凳子上。更衣室里有人在笑，不是恶意，更像是一种——“看你怎么选”的等待。</p>",options:s=>[
    option("先做一个月，再靠表现说话","队内适应+8，体能-8；意志-1",()=>{s.teamFit=clamp((s.teamFit||45)+8);change(s,"fitness",-8);s.attrs.WIL=clamp(s.attrs.WIL-1,1,99);log(s,"story","你收了球。一个月后你在一线队训练里给他送了一脚直塞，他进了，回头看了你一眼，什么也没说。")}),
    option("拒绝，把时间用来加练","意志+1，射术+1；短期首发概率下降",()=>{gain(s,"WIL",1,"pressure");gain(s,"SHO",1,"finish");change(s,"coachFavor",-5);log(s,"story","你去加练了。第二天你到训练场时，发现球已经被人收好了。你没有去问是谁。")})]},
  ...
  {id:"overseas_party",once:true,phase:["overseas"],minAge:17,title:"队友说，放松也是职业的一部分",body:"<p>周六晚上，没有比赛。更衣室里换好衣服，有人拍了拍你的肩膀：<span class='dialogue'>“一起走，所有人都去。”</span>所有人。你去还是不去，都在被观察。</p><p>你站在衣柜前，手机日历上写着明天的恢复计划——冰浴、拉伸、轻量激活。队医用荧光笔画了三条线。</p><p>队友已经在门口等了，回头看了你一眼，笑了一下：<span class='dialogue'>“不来也没事。但来比较好——你知道的。”</span></p><p>他说“你知道的”的时候，语气很轻，像是在教你一个没有人写在纸上的规则。</p>",options:s=>[
    ...
    option("拒绝，独自留在基地","体能+10；队内适应-7，意志+1",()=>{change(s,"fitness",10);s.teamFit=clamp((s.teamFit||45)-7);gain(s,"WIL",1,"pressure");log(s,"story","你回到房间，洗完澡，坐床上刷了会儿手机。零点时你听到楼下有车回来，有人在笑，有人用你的母语喊了句什么，没听清。")})]},
  ...
  {id:"hiv_treatment",once:true,phase:["pro"],minAge:18,title:"治疗不会替你踢球，但能让你继续生活",body:"<p>医生的语气很平，像是在念一份操作手册：<span class='dialogue'>“目前HIV感染已经有规范的治疗方案，只要坚持服药、定期复查，病毒可以被长期抑制。不影响正常生活，不影响工作。”</span></p><p>他把处方笺推过来：<span class='dialogue'>“保密是医疗常规。你的病史只会留在本院的系统里，不会有第二个人知道。”</span>你低头看那张处方笺，上面的药名你从来没有听说过。</p><p>你问了一句：<span class='dialogue'>“如果我间断服药呢？”</span>医生的表情第一次有了变化，很轻，像是失望也像是遗憾：<span class='dialogue'>“耐药之后，治疗选择会越来越少。”</span></p><p>你收好处方笺，站起来之前又坐了回去。诊室的白炽灯很亮，外面走廊里有人在打电话，笑着说明天去哪里吃饭。你坐了很久，医生没有催你。</p>",condition:s=>s.flags.hivDiagnosed&&(s.healthCare||0)<100,options:s=>{const rich=(s.money||0)>=500,arr=[];if(rich)arr.push(option("按医嘱规范治疗（花500万）","病毒长期抑制，身体不再被拖累；意志+2",()=>{addMoney(s,-500);s.healthCare=100;s.flags.hivIntermittent=false;gain(s,"WIL",2,"will");log(s,"good","你负担起了规范治疗。按医嘱服药、定期复查，训练照常。")}));arr.push(option(rich?"省下这笔钱，间断治疗":"负担不起500万，只能间断治疗","每月体能-30、健康风险上升，直到你能规范治疗",()=>{s.flags.hivIntermittent=true;log(s,"bad","药断断续续地吃。身体一天天被拖垮，你只能盼着哪天付得起规范治疗。")},"danger"));return arr}},
  {id:"national_wrong_position",phase:["pro"],minAge:18,title:"国家队要你踢不熟悉的右边翼卫",body:"<p>教练在战术板上点了一下右路的位置：<span class='dialogue'>“你速度快，能解决边路问题。回去看录像，明天合练就按这个打。”</span></p><p>你没有马上回答。你练了十年的位置是前锋，突前、策应、抢点——所有比赛习惯都建立在这个位置上。右边翼卫要回防、要套边、要在攻守转换里不停折返。助理教练在旁边补了一句：<span class='dialogue'>“国家队需要你在这个位置上做出贡献。”</span></p><p>走出战术室，你收到俱乐部教练的消息：<span class='dialogue'>“听说他们要你踢边翼卫？你的训练计划需要调整吗？”</span></p><p>你站在走廊里。一边是国家队的窗口——拒绝了可能再也没有下一次；一边是你花了十年打磨的身体记忆。你把手机锁屏，走廊尽头有人喊你去看录像。</p>",condition:s=>s.national.called,options:s=>[
    option("接受位置，为国出战","国家队适应+15，意志+1；射术成长放缓",()=>{change(s.national,"adapt",15);gain(s,"WIL",1,"national");s.flags.outOfPosition=true;log(s,"story","你花两周恶补边翼卫的跑位录像。第一场热身赛送了两次传中，一次到位，一次出了底线。教练拍你肩说“适应得不错”——你没告诉他，你每晚都在房间对着战术板画跑位。")}),
    option("说明自己只能踢前锋","射术+1；国家队信任-12",()=>{gain(s,"SHO",1,"finish");s.national.adapt=clamp(s.national.adapt-12);log(s,"story","你跟教练谈完，他说“我理解你的想法”，然后把你从首发名单里划掉了。你在看台上看完了那场比赛——你的替补在右翼卫踢了七十二分钟，数据一般，但没有失误。")})]},
  ...
  {id:"study_contract",phase:["campus"],title:"职业试训，和小满的毕业答辩",body:"<p>重庆铜梁龙只给一次三天试训。最后一天，正好是小满的毕业答辩。</p><p>她比你先知道这件事。你还在纠结要不要告诉她的时候，她已经把你的训练日程表打印出来贴在书桌上了：<span class='dialogue'>“三天，第一天适应，第二天对抗，第三天比赛。你第三天早上答辩前出发，来得及赶上下午的对抗。”</span></p><p>你问她答辩几点。她说上午十点。<span class='dialogue'>“你不用回来。”</span>语气跟说“今天可能要下雨”一样平常，<span class='dialogue'>“我准备了很久，不是让你回来听的。”</span>但你听得出来，这句话不是“不需要”，而是“不能说需要”。</p><p>那天晚上你躺在床上，她在隔壁房间背稿。你听见她偶尔停下来，深呼吸，再从头开始。</p><p>第二天早上你起来时，桌上放着一份早餐和一张纸条：<span class='dialogue'>“答辩顺利的话，我中午在校门口等你。”</span>她没有写“如果你在的话”。</p>",portrait:"assets/lin-xiaoman.webp",options:s=>[
    ...
    option("提前离队赶回答辩","感情+15，意志+1；试训成功率下降",()=>{changeLove(s,15);gain(s,"WIL",1,"love");change(s,"scout",-6);log(s,"story","你出现在答辩教室后门时，她刚好念到致谢。她看见你，顿了一拍，继续往下念。结束后她在走廊拉住你，眼睛红了：“你不该回来的。”但拉着你袖子的手没松开。")})]},
  {id:"reconcile",once:true,phase:["firstteam","overseas","campus","pro"],minAge:19,title:"很久没亮的号码又亮了",body:"<p>对话框沉到很下面了。你没有删过聊天记录，但也很久没有往上翻过。</p><p>那天晚上消息弹出来的时候，你刚洗完澡，屏幕上只有一行预览：<span class='dialogue'>“我到你在的城市了，明天下午走。有空的话，一起吃个饭？”</span></p><p>你盯着那条消息看了很久。上一次见面是多久之前了？你甚至想不起最后一次说话的语气。你点进去，看见上一条消息还是她发的——“我理解。”后面什么都没有了。</p><p>你打了一行字：“几点，哪儿？”又删掉。又打：“好久不见。”又删掉。最后你发了一个字：<span class='dialogue'>“好。”</span>她回得很快：<span class='dialogue'>“那我把地址发你。”</span></p><p>地址发过来了，是一家你以前常去的面馆。她什么都没多说，但你知道，那个地方，吃一顿饭的时间比一顿饭本身要长。</p>",portrait:"assets/lin-xiaoman.webp",condition:s=>s.relationship.status==="分手",options:s=>[
    option("赴约，试着重新开始","有机会复合；也可能只是好好告别",()=>{if(chance(.55)){s.relationship.status=phaseOf(s)==="overseas"?"异地":"恋人";s.relationship.love=42;s.relationship.conflict=0;s.flags.breakupQueued=false;change(s,"morale",8);log(s,"good","你们决定重新试试。这次你不想再拿比赛当借口。")}else{change(s,"morale",-4);gain(s,"WIL",1,"will");log(s,"story","一顿饭聊了很多，笑着笑着都明白，回不去了。")}}),
    option("婉拒，把力气留给赛场","状态+4，射术+1；关系仍为分手",()=>{change(s,"form",4);gain(s,"SHO",1,"finish");log(s,"story","从那以后，你每次进球后都会不自觉地往看台某个方向看——即使知道她不在那里。")})]},
  {id:"contract_renewal",phase:["pro"],minAge:19,title:"续约合同摆到了桌上",body:"<p>俱乐部开出的条件是三年，工资涨幅不大，但有绩效奖金。经纪人看了一眼数字，在桌子底下给你发消息：<span class='dialogue'>“这个数低了，我可以再压一压，但可能会惹恼管理层。”</span></p><p>对面的经理在等你签字，笔已经放在纸上了：<span class='dialogue'>“俱乐部很看重你，希望把你作为长期计划的一部分。”</span></p><p>你拿起笔，没有马上签。经理看着你，脸上的笑容很职业，他在等你做选择——涨薪或者安稳。</p><p>经纪人还在看你。你突然意识到，这个房间里没有一个人是在替你想“踢球”这件事。</p>",options:s=>[
    option("强硬要求涨薪","月薪上调；教练信任-8，媒体风险+8",()=>{s.salary=Math.round((s.salary||4)*1.35);change(s,"coachFavor",-8);change(s.risks,"media",8);log(s,"story","经理听完你的要求，把笔收了回去：“那我需要跟上面汇报。”他站起来时椅子腿蹭了一下地板。经纪人随后发来消息：“有戏，但接下来三个月你可能要多等一会儿了。”")}),
    option("接受平稳续约","月薪小涨，教练信任+6；错过一次要价机会",()=>{s.salary=Math.round((s.salary||4)*1.1);change(s,"coachFavor",6);log(s,"story","你签完字，经理跟你握了手，手心是热的：“合作愉快。”走出门时你收到一条消息——另一家俱乐部的球探：“听说你续了？好吧，祝好。”")})]},
  {id:"young_rival",phase:["firstteam","pro"],minAge:18,title:"俱乐部签来一个同位置的新星",body:"<p>他比你小一岁，签进来的时候俱乐部官宣配了两张海报。媒体拿你俩比，标题写的是“新老交替还是良性竞争？”</p><p>训练第一天，他在你对面踢。第一次对抗，他用速度生吃了你一次；第二次你卡住位置没让他转身；第三次他主动过来跟你碰了一下拳头。</p><p>晚上你刷手机，看到他的采访：<span class='dialogue'>“我很尊重前辈，我来是学习的。”</span></p><p>你把手机扣在桌上。你知道那句话说得很得体，也知道他说的是真话。比假话更难消化。</p>",options:s=>[
    option("用更狠的训练回应","射术+1，爆发+1；体能-12，伤病风险上升",()=>{gain(s,"SHO",1,"finish");gain(s,"PAC",1,"burst");change(s,"fitness",-12);s.injury.risk+=6}),
  ...
  {id:"overseas_culture",once:true,phase:["overseas"],title:"更衣室的玩笑你听不懂",body:"<p>不是听不懂单词。是所有人都笑了，你晚了三秒才反应过来那个梗是什么——等你反应过来的时候，笑点已经过了。</p><p>有人注意到了你的延迟，善意地跟你解释了一遍前因后果。你点了点头，笑了一下。但那个笑是表演性的，你知道，他也知道。</p><p>正式训练还没开始。更衣室里大家在换衣服、聊天，开那个你听不懂的玩笑。你系好鞋带，比平时多系了一圈。</p><p>下一次他们再笑的时候，你没有再等那个延迟，低头把护腿板塞进袜子里。你是队里唯一一个没有笑的，但没有人注意到——因为你没有停下手里的动作，你一直在绑鞋带。</p>",options:s=>[
    ...
    option("专注训练，少社交","射术+1，体能+6；队内适应-6",()=>{gain(s,"SHO",1,"finish");change(s,"fitness",6);s.teamFit=clamp((s.teamFit||45)-6)})]},
  {id:"academy_cut",once:true,phase:["academy"],minAge:15,title:"梯队年底要裁掉三个人",body:"<p>周骁在食堂里跟你说的，声音压得很低：<span class='dialogue'>“名单我看到了。年底压缩名单，淘汰三个。”</span>他没有说你在不在上面，但他看着你的眼神已经说明他不知道怎么开口。</p><p>你问他另外两个是谁。他说了一个名字，然后停了一下：<span class='dialogue'>“反正，你自己心里有数。”</span></p><p>那天晚上你加练到操场关灯。保安大爷站在门口等你，手里晃着钥匙串：<span class='dialogue'>“又没人给你开门了是不是。”</span></p><p>你没有回答。你弯腰系鞋带，手指有点抖。</p>",portrait:"assets/coach-zhou.webp",options:s=>[
    option("加倍训练证明自己","射术+1，耐力+1；体能-14，伤病风险上升",()=>{gain(s,"SHO",1,"finish");gain(s,"PHY",1,"stamina");change(s,"fitness",-14);s.injury.risk+=6}),
...
// ===== 新角色：刘队（队长）与苏晚（赞助商千金）=====
{
  ...
  options:s=>[
    option("留下来跟他加练","耐力+1，教练信任+5；体能-8",()=>{gain(s,"PHY",1,"story");change(s,"coachFavor",5);change(s,"fitness",-8);log(s,"story","刘队陪你练到天黑，一句多余的话都没有。")}),
...
const MATCH_PLANS=[
  {id:"box",name:"专注禁区",icon:"◎",desc:"把自己钉在禁区里等球。进球率上去了，参与组织的机会就少了。",
   effects:["进球率↑","助攻率↓","依赖射术"]},
  {id:"deep",name:"回撤组织",icon:"▣",desc:"退到中场接球再前插。评分更稳，但离球门远了一点。",
   effects:["助攻率↑","评分更稳","依赖视野"]},
  {id:"press",name:"疯狂逼抢",icon:"»",desc:"从对方门将脚下就开始压。球队更容易赢，代价是你的身体。",
   effects:["球队胜率↑","体能↓↓","伤病↑"]}
];
...
const MOMENTS=[
  {id:"counter_break",title:"反越位单刀",min:55,max:75,
   ...
   options:[
     {text:"冷静推射",tip:"成功率稳定，依赖射术",stat:"SHO",risk:"safe",style:"box",goal:true,up:.35,down:-.2,
  ...
  {id:"box_scramble",title:"禁区混战",min:20,max:85,cond:s=>s.matchPlan==="box"||styleOf(s,"box")>=1,
   ...
   options:[
     ...
     {text:"贴身垫射",tip:"依赖射术",stat:"SHO",risk:"none",style:"box",goal:true,up:.6,down:-.3,
      win:"你伸脚就捅，球从门将腋下滚进去。不好看，但进了。",fail:"你伸脚够到了球，方向却完全不对。"},
     {text:"做球给后插上",tip:"依赖视野",stat:"PAS",risk:"none",style:"play",assist:true,up:.3,down:-.2,
  ...
  {id:"wing_duel",title:"边路一对一",min:15,max:80,
   ...
   options:[
     {text:"底线传中",tip:"依赖视野",stat:"PAS",risk:"safe",style:"play",assist:true,up:.3,down:-.2,
  ...
  {id:"aerial_duel",title:"角球争顶",min:25,max:85,cond:s=>s.attrs.PHY>=60||styleOf(s,"target")>=1,
   ...
   options:[
     {text:"后点包抄",tip:"依赖对抗",stat:"PHY",risk:"none",style:"target",goal:true,up:.55,down:-.2,
      win:"你绕到后点，几乎无人盯防，头球轻轻一蹭改变了方向。",fail:"你抢到了落点，头球却高出横梁。"},
     {text:"抢前点冲顶",tip:"依赖对抗，风险高",stat:"PHY",risk:"bold",style:"target",goal:true,up:.8,down:-.25,
  ...
  {id:"free_kick",title:"禁区前沿任意球",min:20,max:88,cond:s=>s.attrs.PAS>=55||hasTalent(s,"free_kick"),
   ...
   options:[
     {text:"直接射门",tip:"依赖定位球，风险高",stat:"PAS",risk:"bold",style:"box",goal:true,up:.9,down:-.3,
      win:"皮球越过人墙急速下坠，直挂死角。门将连动都没动。",fail:"球越过了人墙，却被门将侧扑封出。"},
     {text:"传中找中锋",tip:"依赖视野",stat:"PAS",risk:"none",style:"play",assist:true,up:.35,down:-.2,
      win:"你把弧线吊到后点，中锋高高跃起。",fail:"传球被人墙里冲出来的后卫解围。"},
     {text:"短传配合",tip:"依赖视野，稳妥",stat:"PAS",risk:"safe",style:"play",up:.15,down:-.1,
  ...
  {id:"hold_up",title:"背身拿球",min:10,max:80,
   ...
   options:[
     {text:"回敲组织",tip:"依赖视野",stat:"PAS",risk:"safe",style:"play",assist:true,up:.25,down:-.15,
      win:"你把球回敲给插上的中场，他一脚直塞撕开了防线。",fail:"回敲的力量不对，球被断在中场。"},
     {text:"扛住等支援",tip:"依赖耐力，稳妥",stat:"PHY",risk:"safe",style:"target",up:.15,down:-.1,
      win:"你用身体把球护了整整六秒，直到两名队友跑到位。",fail:"你被顶得失去重心，球滚出边线。"},
     {text:"转身强突",tip:"依赖爆发，风险高",stat:"PAC",risk:"bold",style:"burst",goal:true,up:.7,down:-.4,
  ...
  {id:"late_chase",title:"落后追分",min:78,max:88,forced:true,
   ...
   options:[
     {text:"组织最后一攻",tip:"依赖视野",stat:"PAS",risk:"none",style:"play",assist:true,up:.4,down:-.25,
      win:"你没有自己来，一脚过顶球找到了后点完全无人盯防的队友。",fail:"你想找人，但所有人都被盯死了。"},
     {text:"单点强攻",tip:"依赖射术，风险高",stat:"SHO",risk:"bold",style:"box",goal:true,up:1.2,down:-.4,
      win:"你在人堆里硬生生找到一个射门角度，皮球擦着门柱内侧进了。",fail:"你的射门被封堵，反弹出禁区。"},
     {text:"后插上远射",tip:"依赖定位球，成功即经典",stat:"PAS",risk:"bold",style:"box",goal:true,classic:true,up:1.2,down:-.35,
  ...
  {id:"defend_lead",title:"领先保胜果",min:78,max:88,forced:true,
   ...
   options:[
     {text:"回撤参与防守",tip:"依赖耐力，教练最想看到的",stat:"PHY",risk:"safe",style:"target",up:.3,down:-.1,favor:4,fitCost:5,
  ...
  {id:"press_trigger",title:"高位逼抢",min:10,max:70,cond:s=>s.matchPlan==="press",
   ...
   options:[
     {text:"封堵传球线路",tip:"依赖视野",stat:"PAS",risk:"none",style:"play",assist:true,up:.3,down:-.15,favor:3,fitCost:5,
      win:"你切断了门将和后卫之间那条线，逼得他大脚开出，队友轻松拿到二点球。",fail:"你封错了方向，对手从你身后把球做了出去。"},
     {text:"上抢门将出球",tip:"依赖爆发，风险高",stat:"PAC",risk:"bold",style:"burst",goal:true,up:1,down:-.3,fitCost:8,
      win:"你在门将出脚的瞬间把球封了下来，皮球弹进空门。全场哗然。",fail:"你扑了个空，对手就地打起反击。"},
     {text:"保留体力回位",tip:"依赖耐力，稳妥",stat:"PHY",risk:"safe",up:.1,down:-.1,fitGain:4,
  ...
  {id:"through_ball",title:"面向球门接球",min:15,max:80,cond:s=>s.matchPlan==="deep"||styleOf(s,"play")>=1,
   ...
   options:[
     {text:"稳妥横传",tip:"依赖视野，不冒险",stat:"PAS",risk:"safe",style:"play",up:.15,down:-.1,
     ...
     {text:"直接远射",tip:"依赖射术，风险高",stat:"SHO",risk:"bold",style:"box",goal:true,up:.75,down:-.3,
...
function applyMatch(s,report){
  ...
  change(s,"fame",report.goals*1.3+report.assists*.7+(report.rating>=8?2:0));change(s,"fitness",-(hasTalent(s,"engine")?8:12)+(report.fitExtra||0));change(s,"morale",report.rating>=7?3:Math.round(-2*pp));change(s,"coachFavor",(report.rating>=7.5?4:report.rating<6?-3:1)+(report.favorExtra||0));
...
function setRoute(s,route){s.route=route;s.flags.route16=true;if(route==="firstteam"){s.club={name:"重庆铜梁龙",league:"中超",strength:67};s.salary=4;s.relationship.status="恋人";addMoney(s,5);change(s,"fame",5);log(s,"story","你升入重庆铜梁龙一线队，与小满留在同一座城市。")}
  if(route==="overseas"){s.club={name:"Manchester United U18",league:"英超梯队",strength:74};s.salary=3;s.relationship.status="异地";s.language=clamp(s.language+5);change(s,"fame",8);change(s,"morale",-4);log(s,"story","你飞往英格兰的青训营。临行前你和小满约好试试异地，谁也没提“分手”——从此隔着七个小时的时差。")}
...
function applyAging(s){const d=diffOf(s),age=ageInfo(s).age;if(age<d.decayAge)return;const yrs=age-d.decayAge+1,m=d.soft;const drop=base=>Math.max(0,(base+yrs*.7)*m*rndFloat(Math.random,.6,1.3));
  s.attrs.PAC=clamp(s.attrs.PAC-drop(1.55),1,99);s.attrs.PHY=clamp(s.attrs.PHY-drop(1.2),1,99);
...
function riskSettlement(s){
  ...
  if(s.flags.hivDiagnosed&&s.flags.hivIntermittent){change(s,"fitness",-30);change(s.risks,"health",8)}
}
...
function advanceMonth(force=false){if(!S||modalBusy||S.retired)return;if(S.actionPoints>0&&!force){const n=S.actionPoints;enqueueDecision({title:"还有执行点没有使用",body:`本月还剩 <b>${n}</b> 点。剩下的时间会自动用来休息：<b>体能 +${6*n}</b>、<b>伤病风险 −${3*n}</b>，但不会有任何属性成长。`,options:[option("继续安排本月","返回行动面板",()=>{}),option("休息，进入下个月",`体能+${6*n}，伤病风险−${3*n}`,()=>setTimeout(()=>advanceMonth(true),120))]},"时间确认");return}
  ...
  S.totalMonth++;S.actionPoints=3;S.actionUsage={};S.combosHit=[];change(S,"fitness",16+Math.max(0,Math.round((60-S.fitness)*0.7)));change(S,"morale",1);change(S,"form",Math.round((52-S.form)*0.22));

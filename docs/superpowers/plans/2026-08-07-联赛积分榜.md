...
// 玩家一加总就会发现这榜是假的。必须真实两两配对。
{
  ...
  .forEach(([label,club,route,month])=>{
    ...
    /* 轮空的稳健判据是「总场次」，不是「有几支队少打」——
       同一支队可以在不同轮次重复轮空，那时少打的队数就不等于轮数了。
       实测 20 个赛季里有 4 个会出现重复轮空。 */
    const sumP=T.reduce((n,x)=>n+x.p,0);
    const expect=N%2===1?3*(N-1):3*N;
    assert.equal(sumP,expect,
      `${label} 是${N}队，3轮后总场次应为 ${expect}（奇数队每轮一人轮空），实际 ${sumP}`);
    T.forEach(x=>assert.ok(x.p<=3,`${label} ${x.name} 打了 ${x.p} 场，超过了3轮`));
  });
}

...
文件里有 **244 个中文弯引号 `“”`** 嵌在 `"..."` 字符串里。任何碰引号字符的批量操作都会把文件搞崩，而且崩得很隐蔽。**只用 Edit 工具做精确字符串替换，每改一处跑一次 `node --check app.js`。** 改完核对 `grep -o "“" app.js | wc -l` 仍为 244。
...
// 排序：积分 → 净胜球 → 进球 → 队名
{
  ...
  assert.deepEqual([...out],["丙","甲","丁","乙"],
    `排序应为 积分→净胜球→进球→队名，实际 ${[...out].join(",")}`);
}
```
